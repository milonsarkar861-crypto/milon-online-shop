...
async function allProducts(env){
  ...
  if(!arr.length){
    for(const p of DEFAULT_PRODUCTS){
      await env.STORE.put(key(p.id),JSON.stringify(p));
    }
...
async function handleApi(req,env,url){
  ...
  if(url.pathname==='/api/admin/login' && req.method==='POST'){
    ...
    if(body.password!==ADMIN_PASSWORD){
      return json({error:'Password ভুল'},401);
    }

    if(!env.STORE){
      return json({error:'STORE binding missing'},500);
    }
    ...
    return new Response(
      JSON.stringify({ok:true}),
      {
        ...
        headers:{
          ...
          'cache-control':'no-store',
          'set-cookie':
            `milon_admin=${token}; Max-Age=2592000; Path=/; HttpOnly; Secure; SameSite=Lax`
        }
      }
    );
  }
  ...
  if(url.pathname==='/api/admin/logout' && req.method==='POST'){
    ...
    if(m&&env.STORE){
      await env.STORE.delete('session:'+m[1]);
    }
    ...
    return new Response(
      JSON.stringify({ok:true}),
      {
        ...
        headers:{
          'content-type':'application/json;charset=UTF-8',
          'set-cookie':
            'milon_admin=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Lax'
        }
      }
    );
  }
  ...
  if(url.pathname==='/api/products' && req.method==='GET'){
    return json({
      products:(await allProducts(env)).filter(
        p=>p.active&&Number(p.stock)>0&&Number(p.price)>0
      )
    });
  }
  ...
  if(url.pathname==='/api/admin/products' && req.method==='GET'){
    ...
    return json({
      products:await allProducts(env)
    });
  }
  ...
  if(url.pathname==='/api/admin/products' && req.method==='POST'){
    ...
    if(!p.name){
      return json({error:'পণ্যের নাম দিন'},400);
    }
    ...
    const product={
      id,
      name:String(p.name),
      category:String(p.category||'সাধারণ'),
      desc:String(p.desc||''),
      ...
      sampleTest:String(p.sampleTest||'Pending'),
  ...
  if(url.pathname==='/api/admin/orders' && req.method==='GET'){
    ...
    return json({
      orders:(await listAll(env,'order:'))
        .sort((a,b)=>
          String(b.createdAt)
